...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        // Queue<ParComponent> object is a ConcurrentLinkedQueue so 
        // safe to access nodesLoaded from multiple threads concurrently
        // without additional synchronization
        while (!nodesLoaded.isEmpty()){
            loopNode = nodesLoaded.poll();

            // 1. Check loop node is available and not dead

            // optimistic concurrency where try to see if available and if
            // not then continue to next iteration
            ...
            final Edge<ParComponent> loopNodeEdge = loopNode.getMinEdge();
            if (loopNodeEdge == null){
                solution.setSolution(loopNode);
                break; // done we've contracted the graph to a single node
            }

            // 2. Get loop node neighbour

            final ParComponent loopNodeNeighbour = loopNodeEdge.getOther(loopNode);

            // 3. Check if neighbour is available and not dead

            // see if lock on other available, if not release first node
            if (!loopNodeNeighbour.lock.tryLock()){
                loopNode.lock.unlock();
                nodesLoaded.add(loopNode);
                continue;
            } 

            // see if lock has already been processed, if so then release first node
            if (loopNodeNeighbour.isDead) {
                loopNode.lock.unlock();
                loopNodeNeighbour.lock.unlock();
                nodesLoaded.add(loopNode);
                continue;
            }

            // 4. Merge loopNode, unlock it and neighbour

            loopNodeNeighbour.isDead = true;
            // merge node other into node loopNode
            loopNode.merge(loopNodeNeighbour, loopNodeEdge.weight());
            loopNode.lock.unlock();
            loopNodeNeighbour.lock.unlock();
    ...
    public static final class ParComponent extends Component<ParComponent> {
        ...
        /*
         * We need a try lock for ParComponent
         */
